...
class OpticsAppController:
    def __init__(self, server):
        ...
        self.state.param_reflection_min = 0
        self.state.param_reflection_max = 0
        self.state.param_refraction_min = 0
        self.state.param_refraction_max = 0
        self.state.param_absorption_min = 0
        self.state.param_absorption_max = 0
    ...
    def _load_params_to_state(self, obj_entry):
        ...
        # Загружаем диапазоны (если есть)
        def range_to_state(key):
            val = p.get(key, None)
            if val is None:
                return "", ""
            return str(val[0]) if val[0] != -np.inf else "", str(val[1]) if val[1] != np.inf else ""
        ref_min, ref_max = range_to_state("reflection_range")
        self.state.param_reflection_min = ref_min
        self.state.param_reflection_max = ref_max
        refr_min, refr_max = range_to_state("refraction_range")
        self.state.param_refraction_min = refr_min
        self.state.param_refraction_max = refr_max
        abs_min, abs_max = range_to_state("absorption_range")
        self.state.param_absorption_min = abs_min
        self.state.param_absorption_max = abs_max
    ...
    def update_selected_object(self, *args, **kwargs):
        ...
        def state_to_range(min_str, max_str):
            try:
                mn = float(min_str) if min_str.strip() != "" else -np.inf
                mx = float(max_str) if max_str.strip() != "" else np.inf
                if mn == -np.inf and mx == np.inf:
                    return None
                if mn == -np.inf:
                    return (None, mx) if mx != np.inf else None
                if mx == np.inf:
                    return (mn, None) if mn != -np.inf else None
                return (mn, mx)
            except ValueError:
                return None

        p["reflection_range"] = state_to_range(self.state.param_reflection_min, self.state.param_reflection_max)
        p["refraction_range"] = state_to_range(self.state.param_refraction_min, self.state.param_refraction_max)
        p["absorption_range"] = state_to_range(self.state.param_absorption_min, self.state.param_absorption_max)
...
params_to_watch = [
    "param_n", "param_R1", "param_R2", "param_f_target", "param_thickness", "param_edge_radius",
    "param_num_rays", "param_min_offset", "param_max_offset", "param_wavelength", "param_mesh_path",
    "param_reflection_min", "param_reflection_max",
    "param_refraction_min", "param_refraction_max",
    "param_absorption_min", "param_absorption_max"
]
for param in params_to_watch:
            ...
            with vuetify.VRow(no_gutters=True, style="height: 100%;"):
                ...
                with vuetify.VCol(cols=3, classes="pa-4 bg-grey-darken-4",
                    ...
                    with vuetify.VContainer(
                            v_if="selected_object_type == 'lens' or selected_object_type == 'hyperbolic_lens' or selected_object_type == 'mesh'",
                            class_="pa-0"):
                        vuetify.VDivider(class_="my-4")
                        vuetify.VListSubheader("Диапазоны взаимодействия", class_="px-0")
                        # Отражение
                        with vuetify.VRow(no_gutters=True, align="center"):
                            with vuetify.VCol(cols=6):
                                vuetify.VTextField(v_model="param_reflection_min", label="Отражение min", type="number",
                                               dense=True, step=0.1, placeholder="∞")
                        with vuetify.VCol(cols=6):
                            vuetify.VTextField(v_model="param_reflection_max", label="Отражение max", type="number",
                                           dense=True, step=0.1, placeholder="∞")
                        # Преломление
                        with vuetify.VRow(no_gutters=True, align="center"):
                            with vuetify.VCol(cols=6):
                                vuetify.VTextField(v_model="param_refraction_min", label="Преломление min", type="number",
                                               dense=True, step=0.1, placeholder="∞")
                        with vuetify.VCol(cols=6):
                            vuetify.VTextField(v_model="param_refraction_max", label="Преломление max", type="number", dense=True,
                                           step=0.1, placeholder="∞")
                        # Поглощение
                        with vuetify.VRow(no_gutters=True, align="center"):
                            with vuetify.VCol(cols=6):
                                vuetify.VTextField(v_model="param_absorption_min", label="Поглощение min", type="number", dense=True,
                                               step=0.1, placeholder="∞")
                        with vuetify.VCol(cols=6):
                            vuetify.VTextField(v_model="param_absorption_max", label="Поглощение max", type="number", dense=True, step=0.1,
                                           placeholder="∞")
