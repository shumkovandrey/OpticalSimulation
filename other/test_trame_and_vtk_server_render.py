...
# ИСПРАВЛЕНО: Импортируем официальную функцию plotter_ui вместо отсутствующего viewer
...
class OpticsWebController:
    def __init__(self, server):
        ...
        # Инициализируем плоттер PyVista строго в фоновом (off_screen) режиме
        self.plotter = pv.Plotter(off_screen=True)
        self.plotter.set_background("#2A2E35")

        # Исправлено: Убрали add_axes(), чтобы не было синего пятна в углу экрана
    ...
    def rebuild_scene(self):
        """Перерисовка 3D объектов и луча на стороне сервера."""
        ...
        # ВАЖНО: Вызываем встроенный триггер обновления вида PyVista
...
# =============================================================================
# 3. ВЕБ-ИНТЕРФЕЙС (Серверный рендеринг в сетке VRow/VCol)
# =============================================================================
...
with SinglePageLayout(server) as layout:
    layout.title.set_text("Оптический 3D Симулятор (Серверный рендеринг)")
    ...
    with layout.content:
        with vuetify.VContainer(fluid=True, class_="pa-0 fill-height", style="height: 100vh; overflow: hidden;"):
            with vuetify.VRow(no_gutters=True, class_="fill-height", style="height: 100%;"):
                # КОЛОНКА 1 (Слева): Панель управления
                ...
                # КОЛОНКА 2 (Справа): 3D Окно СЕРВЕРНОГО рендеринга
                with vuetify.VCol(cols=9, class_="fill-height", style="height: 100%; position: relative;"):
                    # ИСПРАВЛЕНО: Используем фабричный вьюер PyVista, настроенный на Remote-режим.
                    # Он автоматически открывает правильный WebSocket протокол, убирая ошибку 405.
                    # ИСПРАВЛЕНО: Используем стандартную функцию plotter_ui с параметром mode="server".
                    # add_menu=False убирает стандартную верхнюю панель PyVista.
                    ui_view = plotter_ui(app_controller.plotter, mode="server", add_menu=False)
                    app_controller.ctrl.view_update = ui_view.update

# Первичное построение сцены на сервере
app_controller.rebuild_scene()
app_controller.plotter.view_isometric()

if __name__ == "__main__":
    # Запуск на выделенном свободном порту 8085
